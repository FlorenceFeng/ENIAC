...
#from DeepRL.baselines.common.atari_wrappers import make_atari, wrap_deepmind
#from DeepRL.baselines.common.atari_wrappers import FrameStack as FrameStack_
#from DeepRL.baselines.common.vec_env.subproc_vec_env import SubprocVecEnv, VecEnv
...
class DiabolicalLockMaze(object):
    ...
    def __init__(self, horizon, seed=1, noise='bernoulli', n_locks=2):
        self.horizon = horizon
        self.num_actions = 10
        assert n_locks <= self.num_actions
        self.n_locks = n_locks
        self.locks = [build_env_homer(horizon=horizon-1, seed=seed, noise=noise), build_env_homer(horizon=horizon-1, seed=seed*10, noise=noise)]
        self.optimal_reward = 5.0
        self.locks[0].env.optimal_reward = self.optimal_reward
        self.locks[1].env.optimal_reward = 2.0

        self.reward_range = (0, self.optimal_reward)
        self.metadata = None
        self.action_space = gym.spaces.Discrete(self.num_actions)
        self.observation_space = self.locks[0].observation_space

    
    def reset(self):
        self.h = 0
        obs = np.ones(self.observation_space.shape[0])
        return obs
...
class DiabolicalLockMaze(object):
    ...
    def __init__(self, horizon, seed=1, noise='bernoulli', n_locks=2, use_one_hot=False):
        self.horizon = horizon
        self.n_states = 3
        self.num_actions = 10
        assert n_locks <= self.num_actions
        ...
        if self.use_one_hot:
            self.n_features = 3*self.horizon*self.n_states
        else:
    ...
    def one_hot(self, x):
        s, h, l = x
        j = np.ravel_multi_index((s, l, h), (3, 3, self.horizon))
...
class LongHallway(object):
    ...
    def step(self, action):
        if action == 0 and self.pos < self.length - 1:
            self.pos += 1
        elif action == 1:
            pass
        ...
        if self.pos == 0:
            ...
            reward = self.optimal_reward / 2
        else:
            reward = -1.0/(self.horizon - 1)
        self.h += 1
        done = (self.h == self.horizon + 10)
        info = {}
        return obs, reward, done, info
...
def build_env_homer(horizon=10, noise='bernoulli', seed=1):
    import homer_envs
    config = homer_envs.default_config.default_config()
    config['horizon'] = horizon
    config['noise'] = noise
    config['save_trace'] = False
#    config['anti_shaping_reward'] = config['anti_shaping_reward2'] = 0
...
def make_env(env_id, seed, rank, episode_life=True, horizon=4, noise='bernoulli', dimension=30):
    def _thunk():
        random_seed(seed)
        if 'diabcombolock' == env_id:
            env = build_env_homer(horizon=horizon, seed=seed)
        elif 'hallway' == env_id:
            env = LongHallway(horizon=horizon)
        elif 'diabcombolockhallway' == env_id:
            env = DiabolicalLockMaze(horizon=horizon, seed=seed, noise=noise, n_locks=2)            
        elif 'Lock-v0' in env_id:
            # Build combination lock environment
            import Environments
            env = gym.make('Lock-v0')
            ep_dict = {'horizon': horizon,
                       'dimension': dimension,
                       'switch': 0.1,
                       'tabular': False}
...
class Task:
    def __init__(self,
        ...
        if log_dir is not None:
            mkdir(log_dir)
        if 'Lock-v0' in name:
            env = make_env(name, seed, 0, episode_life, horizon=horizon, dimension = dimension)
            envs = [env for i in range(num_envs)]
        elif 'diabcombolock' == name:
            env = make_env(name, seed, 0, episode_life, horizon=horizon, dimension = dimension)
            envs = [env for i in range(num_envs)]
        elif 'hallway' == name:
            env = make_env(name, seed, 0, episode_life, horizon=horizon)
            envs = [env for i in range(num_envs)]
        elif 'diabcombolockhallway' in name:
