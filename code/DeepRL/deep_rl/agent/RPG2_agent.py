    ...
    def compute_reward_bonus(self, states, actions = None):
        if self.config.bonus == 'rnd':
            ...
            for s in self.config.state_normalizer(states):
                s = tuple(s)
                if not s in self.density_model['explore-exploit'].keys():
                    cnts = 0
                else:
                    cnts = self.density_model['explore-exploit'][s]
    ...
    def gather_trajectories(self, roll_in=True, add_bonus_reward=True, debug=False, mode=None, record_return=False):
        ...
        for i in range(roll_out_length):
            ...
            if add_bonus_reward:
                ...
                reward_bonus = reward_bonus * self.config.rmax * self.config.horizon
                if not roll_in:
                    total_rewards['bonus'] += reward_bonus.mean().item()
                    total_rewards['real'] += rewards.mean()
                rewards += self.config.bonus_coeff*to_np(reward_bonus)
#                assert(all(rewards >= 0))
        ...
        if random.random() < 0.01 and not roll_in:
            ...
            print(ri*self.config.horizon, re)
...
#        assert(np.array(terminals).all()) # debug
    ...
    def compute_kernel(self, states, actions = None):
        ...
        actions = tensor(actions).to(Config.DEVICE)
        state_actions = torch.cat((tensor(states).to(Config.DEVICE), actions), dim=1)
    ...
    def update_density_model(self, mode=None):
        ...
        if self.config.bonus == 'rnd':
            ...
            n, d = phi.shape
            sigma = torch.mm(phi.t(), phi) + self.config.ridge*torch.eye(d).to(Config.DEVICE).float()
            ...
            for i, states in enumerate(replay_buffer):
                ...
                covariance_matrices.append(torch.diag(sums) + torch.eye(len(unique_states))).float()
        ...
        if mode == 'explore-exploit': self.optimize_policy_mixture_weights(covariance_matrices)
...
#                visitations.append(self.compute_state_visitations(self.replay_buffer_infos[mode][i]))
    ...
    def optimize_policy_mixture_weights(self, covariance_matrices):
        ...
        if N == 1:
            ...
            for i in range(5000):
                ...
                for n in range(N):
                    sigma_weighted_sum += F.softmax(self.log_alphas, dim=0)[n]*covariance_matrices[n].cpu()
    ...
    def optimize_policy(self):
        if self.config.init_new_policy == 1:
            self.initialize_new_policy('explore-exploit')
...
#        self.time('optimizing policy')
    ...
    #extra helpers:
    def continous_uniform_prob(self):
        act_max = self.task.action_space.high
        act_min = self.task.action_space.low
    ...
    def uniform_sample_cont_random_acts(self, N):
        acts = []
        for i in range(N):
            act = self.task.action_space.sample()
            acts.append(act)
